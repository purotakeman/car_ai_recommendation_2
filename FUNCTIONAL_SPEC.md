...
| POST | `/api/cars/batch` | 複数車両データ一括取得 | 不要 |
...
#### 3.2.3 POST `/api/cars/batch`

**説明**: 複数の車両IDを一括で受け取り、それぞれの詳細情報を返す。お気に入り一覧の表示などで使用。YouTube Data APIと連携しサムネイル情報も付与する。
...
| SCREEN_003 | お気に入りページ | `/favorites` | お気に入り車両一覧（最大9件） |
...
**インタラクション**:
- タブ切り替えでコンテンツ表示切り替え
- お気に入りボタンでローカルストレージに保存（ハートの色が変化）
- シェアボタン（将来実装）

---

#### 5.3.3 SCREEN_003: お気に入りページ

**レイアウト構成**:
- 検索結果画面と同様のヘッダー・フッターを保持
- 中央にお気に入り一覧エリアを配置
- **3x3のグリッドレイアウト**（最大9件表示）を採用
- 登録車両がない場合は「お気に入りはまだありません」等のメッセージを表示

**主要機能**:
- **バッチ取得**: 表示開始時に `localStorage` から取得したIDリストを基に `/api/cars/batch` からデータを取得。
- **詳細カード表示**: 検索結果と同一の、サムネイルやYouTube連携を含んだ詳細な車両カードを表示。
- **リアルタイム削除**: お気に入り解除ボタン（塗りつぶしハート）をクリックすると、即座にカードが消え、localStorage も更新される。
...
<div class="car-card">
  ...
  <div class="car-body">
    <div class="card-media">
       <!-- YouTubeサムネイル または プレースホルダー画像 -->
    </div>
    <div class="car-info">
      <!-- スペック情報（アイコン付き） -->
    </div>
    <div class="car-actions">
      <a href="/car/{id}" class="action-button">詳細</a>
